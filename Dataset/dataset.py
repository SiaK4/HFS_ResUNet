...
"""
The code is a slightly modified version the source dataset code:  https://github.com/HPCForge/BubbleML.git
"""
...
class HDF5ConcatDataset(ConcatDataset):
    ...
    def find_min_temp(self):
        return min(d.find_min_temp() for d in self.datasets)
...
class HDF5Dataset(Dataset):
    ...
    def find_min_temp(self):
        return self._data['temp'].abs().min()
...
class TempInputDataset(HDF5Dataset):
    ...
    def __getitem__(self, timestep):
        ...
        dfun = torch.stack([self._get_dfun(timestep + k) for k in range(self.time_window + self.future_window)], dim=0)
...
# val_dataset = HDF5ConcatDataset(TempInputDataset(file, steady_time=30, use_coords=False, time_window=5, future_window=5) for file in validation_files)

...
class HFS(nn.Module):
    ...
    def decompose_frequency(self,x):
        ...
        transition_width = 2
        x_coord = np.arange(cols) - center_col
        y_coord = np.arange(rows) - center_row
        X_coord, Y_coord = np.meshgrid(x_coord,y_coord)
        distance = torch.sqrt(torch.tensor(X_coord)**2 + torch.tensor(Y_coord)**2)
        
        mask_low = 1/(1 + torch.exp((distance - self.cutoff)/transition_width))
